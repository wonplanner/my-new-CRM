...
// Fixed: Added RefreshCw to the imports from lucide-react
import { 
  BellRing, Megaphone, Plus, Trash2, Calendar, 
  Users, FileText, CheckCircle2, ChevronRight, Settings, Edit3, X, Sparkles, Wand2, Loader2, Clock, CheckCircle, RefreshCw
} from 'lucide-react';
...
const DashboardView: React.FC<DashboardViewProps> = ({ 
  ...
  aiInsights, onGenerateAiInsights, isAiLoading, lastAiUpdate
}) => {
  ...
  const steps = ["데이터 암호화 중...", "영업 벡터 추출 중...", "AI 브레인 분석 중...", "리포트 생성 완료"];
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section className="bg-white rounded-[40px] p-8 shadow-sm border border-indigo-100 relative overflow-hidden group h-full flex flex-col min-h-[320px]">
          <div className="absolute top-0 right-0 p-6 opacity-5 -mr-4 -mt-4 pointer-events-none">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              {lastAiUpdate && !isAiLoading && (
                <span className="text-[10px] font-bold text-gray-300 flex items-center gap-1">
                  <Clock size={10} /> {lastAiUpdate}
                </span>
              )}
              <button 
                disabled={isAiLoading}
                onClick={onGenerateAiInsights}
                className="text-indigo-500 hover:text-indigo-700 transition-all active:scale-90 disabled:opacity-50"
              >
            ...
            {isAiLoading ? (
              <div className="h-full flex flex-col items-center justify-center space-y-4 animate-in fade-in">
                ...
                <div className="text-center">
                  <p className="text-xs font-black text-indigo-600 animate-pulse">{steps[loadingStep]}</p>
                  <p className="text-[10px] text-gray-400 mt-1 font-bold">인슈어플래너 클라우드 엔진 연동 중</p>
                </div>
              </div>
            ) : aiInsights ? (
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                <div className="mt-4 flex items-center justify-between px-1">
                  <div className="flex items-center gap-2 text-[10px] font-bold text-indigo-400">
                    ...
                    <span>분석 로그: 정상 (Data Vector {totalCustomers})</span>
                  </div>
                </div>
              </div>
            ) : (
              <div className="text-center py-10 flex flex-col items-center justify-center">
                <div className="w-16 h-16 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-200 mb-4">
                  <Sparkles size={32} />
                </div>
                <p className="text-sm text-gray-400 mb-6 font-bold leading-relaxed">
                  전체 고객 데이터를 분석하여<br/>최적화된 상담 리스트를 생성할까요?
                </p>
                <button 
                  onClick={onGenerateAiInsights}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-2xl text-sm font-black shadow-xl shadow-indigo-500/20 active:scale-95 transition-all"
                >
                  AI 분석 보고서 생성
                </button>
              </div>
        ...
        <section className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-[40px] shadow-2xl shadow-blue-500/20 text-white overflow-hidden h-full flex flex-col justify-between min-h-[320px]">
          <div className="relative z-10 space-y-4">
            ...
            <div className="min-h-[120px] flex items-center">
              {activeNotice ? (
                <p className="text-xl md:text-2xl font-bold leading-tight animate-in slide-in-from-bottom-2 duration-500">
      ...
      <section className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm flex flex-col md:flex-row items-center gap-6 group hover:border-blue-200 transition-all cursor-pointer">
        <div className="w-16 h-16 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400 shrink-0 group-hover:bg-blue-50 group-hover:text-blue-500 transition-all">
          <Calendar size={32} />
        </div>
        ...
        <div className="px-6 py-3 bg-gray-900 text-white rounded-2xl text-sm font-bold flex items-center gap-2 group-hover:bg-blue-600 transition-all">
          ...
          <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {notices.length === 0 ? (
                  <p className="text-center text-gray-400 py-10 text-sm font-bold">리마인더가 비어 있습니다.</p>
                ) : (
                  notices.map(notice => (
                    <div key={notice.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group hover:border-blue-200 transition-all">
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100 text-center">
              <button onClick={() => setShowNoticeEditor(false)} className="bg-gray-900 text-white px-10 py-3.5 rounded-2xl font-bold shadow-xl active:scale-95 transition-all">완료</button>
            </div>
          </div>
        </div>
      )}
