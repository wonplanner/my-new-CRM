...
import { Customer, CustomerStatus, Contract } from '../types';
import { X, Save, User, Phone, MapPin, Calendar, Briefcase, ShieldCheck } from 'lucide-react';
            ...
            <div className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div className="relative">
                 ...
                 {showSuggestions && suggestions.length > 0 && (
                   <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl z-30 overflow-hidden shadow-blue-500/10 border-blue-100">
                     {suggestions.map(s => (
                       <button key={s.id} type="button" onClick={() => {setFormData({...formData, name: s.name, phone: s.phone}); setShowSuggestions(false);}} className="w-full px-4 py-3 text-left hover:bg-gray-50 flex items-center justify-between border-b last:border-0 transition-colors">
                         ...
                         <span className="text-[10px] font-black text-blue-500 bg-blue-50 px-2 py-1 rounded-lg">불러오기</span>
                       </button>
                     ))}
                   </div>
                 )}
               ...
               <div className="md:col-span-2">
                  ...
                  <div className="flex gap-2 p-1 bg-gray-50 rounded-2xl border border-gray-100">
                    {Object.values(CustomerStatus).map(status => (
                      <button key={status} type="button" onClick={() => setFormData({...formData, status})} className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all ${formData.status === status ? 'bg-white text-blue-600 shadow-sm border border-gray-100' : 'text-gray-400 hover:text-gray-600'}`}>{status}</button>
                    ))}
                  </div>
               </div>
            </div>
  ...
  <div>
    ...
    <div className="relative">
      ...
      <input 
        ...
        className="w-full bg-gray-50 border border-gray-100 rounded-2xl py-3.5 pl-12 pr-4 text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" 
        value={value} 
