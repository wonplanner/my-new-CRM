...
import { 
  ...
  MapPin, Calendar, Briefcase, Plus, ExternalLink, PhoneCall, X, Save, Tag as TagIcon, MessageCircle, Clock, CreditCard, Banknote, Share2, HeartPulse, Stethoscope, ShieldCheck, Wallet, Info
} from 'lucide-react';
...
const PAYMENT_METHODS = ['자동이체', '신용카드', '지로', '직접수납'];
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 custom-scrollbar">
        ...
        {activeTab === 'contract' && (
          <div className="space-y-6 animate-in fade-in duration-300 max-w-3xl mx-auto">
            ...
            {showContractForm && (
              <div className="bg-white p-8 rounded-[40px] border-2 border-blue-500/10 shadow-2xl mb-8 animate-in slide-in-from-top-4 overflow-hidden relative">
                ...
                <div className="flex justify-between items-center mb-8 pl-4">
                  <div>
                    <h4 className="font-black text-gray-900 text-xl tracking-tight">
                      {editingContractId ? '계약 정보 수정' : '신규 계약 등록'}
                    </h4>
                    <p className="text-[10px] text-gray-400 font-bold mt-1">상세한 결제 정보를 입력하면 관리가 더욱 편리해집니다.</p>
                  </div>
                  <button onClick={() => setShowContractForm(false)} className="p-2 bg-gray-50 rounded-full text-gray-400 hover:text-gray-600 transition-colors"><X size={20} /></button>
                </div>
                
                <div className="space-y-10 pl-4">
                  <div className="space-y-6">
                    <label className="text-[10px] font-black text-blue-500 uppercase tracking-[0.15em] px-1 flex items-center gap-2">
                      <FileText size={12} /> 상품 및 보장 정보
                    </label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <FormInputSmall icon={ShieldCheck} label="보험사" placeholder="삼성화재, 메리츠 등" value={contractFormData.insurer || ''} onChange={v => setContractFormData({...contractFormData, insurer: v})}/>
                      <FormInputSmall icon={HeartPulse} label="상품명" placeholder="통합보험, 운전자보험 등" value={contractFormData.productName || ''} onChange={v => setContractFormData({...contractFormData, productName: v})}/>
                      <FormInputSmall icon={Banknote} label="월 보험료" placeholder="0" type="number" value={contractFormData.premium?.toString() || ''} onChange={v => setContractFormData({...contractFormData, premium: parseInt(v) || 0})}/>
                      <FormInputSmall icon={Calendar} label="보험 개시일" type="date" value={contractFormData.startDate || ''} onChange={v => setContractFormData({...contractFormData, startDate: v})}/>
                    </div>
                  </div>

                  <div className="space-y-6">
                    <label className="text-[10px] font-black text-blue-500 uppercase tracking-[0.15em] px-1 flex items-center gap-2">
                      <CreditCard size={12} /> 수납 및 결제 정보
                    </label>
                    <div className="space-y-5">
                      <div>
                        <label className="text-[10px] font-black text-gray-400 mb-3 block px-1 uppercase tracking-widest">결제 방식 선택</label>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          {PAYMENT_METHODS.map(method => (
                            <button 
                              key={method}
                              type="button"
                              onClick={() => setContractFormData({...contractFormData, paymentMethod: method})}
                              className={`py-2.5 rounded-xl text-[11px] font-black border transition-all ${contractFormData.paymentMethod === method ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-100 hover:border-gray-200'}`}
                            >
                              {method}
                            </button>
                          ))}
                      ...
                      <FormInputSmall icon={Wallet} label="결제 상세 (은행/카드번호 등)" placeholder="우리은행 1002-***-**** / 현대카드 등" value={contractFormData.paymentDetails || ''} onChange={v => setContractFormData({...contractFormData, paymentDetails: v})}/>
                    </div>
                  </div>

                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-blue-500 uppercase tracking-[0.15em] px-1 flex items-center gap-2">
                      <TagIcon size={12} /> 분류 태그
                    </label>
                  ...
                  <div className="flex gap-3 pt-4">
                    <button onClick={() => setShowContractForm(false)} className="flex-1 bg-gray-100 text-gray-500 py-4 rounded-[24px] text-sm font-black active:scale-[0.98] transition-all">취소</button>
                    <button onClick={handleSaveContract} className="flex-[2] bg-blue-600 text-white py-4 rounded-[24px] text-sm font-black shadow-xl shadow-blue-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                      <Save size={18} /> {editingContractId ? '수정 내용 저장' : '신규 계약 저장'}
                    </button>
                  </div>
                </div>
              </div>
              ...
              {customer.contracts.length === 0 ? (
                ...
                customer.contracts.map(contract => (
                  <div key={contract.id} className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 group relative overflow-hidden hover:shadow-xl transition-all">
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-50 grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="text-[10px] text-gray-400 font-bold">결제 정보 ({contract.paymentMethod || '미지정'})</p><p className="text-sm font-black text-gray-700 truncate max-w-[150px]">{contract.paymentDetails || '상세 정보 없음'}</p></div>
                      </div>
                    </div>
                  </div>
                ))
...
const FormInputSmall = ({ icon: Icon, label, placeholder, value, onChange, type = "text" }: { icon?: any, label: string, placeholder?: string, value: string, onChange: (v: string) => void, type?: string }) => (
  <div>
    ...
    <div className="relative">
      ...
      <input 
        ...
        className={`w-full p-4 rounded-[20px] bg-gray-50 border border-gray-100 text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/10 transition-all ${Icon ? 'pl-11' : ''}`}
        value={value} 
